...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
         ...
         */
        if(shotgun){
            float angleIncrement = shotgunSpread / (shotgunBullets - 1);

            for (int i = 0; i < shotgunBullets; i++)
            {
                Quaternion bulletRotation = Quaternion.Euler(0.0f, 0.0f, -(shotgunSpread / 2) + (i * angleIncrement));
                SpawnBullet(transform.position, transform.rotation*bulletRotation);
            }
